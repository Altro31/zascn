	...
	const queryItems = useMemo(
		() =>
			data?.pages.flatMap((page) =>
				Array.isArray(page?.data?.data) ? (page.data.data as T[]) : [],
			) ?? [],
		[data],
	);
								...
								<div className='bg-muted/40 flex flex-col gap-2 rounded-lg border px-3 py-2 md:flex-row md:items-center md:justify-between'>
									...
									<div className='flex gap-2'>
										<button
											...
											className='bg-primary rounded-md px-3 py-1 text-[11px] text-white'
										>
										...
										<button
											...
											className='rounded-md bg-slate-200 px-3 py-1 text-[11px] dark:bg-slate-700'
										>
											Limpiar todo
										</button>
									</div>
...
const ItemRow = React.memo(
	({ id, isSelected, onToggle, label, description }: any) => {
		return (
			<div
				onClick={() => onToggle(id)}
